...
public class CircuitoMaritimo {
    ...
    private double tiempoDeRecorridoPorMilla;  // minutos

    public CircuitoMaritimo(String nombre, double tiempoDeRecorridoPorMilla) {
        this.nombre = nombre;
    ...
    public double distanciaEntre(TerminalPortuaria origen, TerminalPortuaria destino) {
        ...
        while (i != indexDestino) {
            ...
            i = (i + 1) % puertos.size(); // avanzando circularmente
        }

        return distancia;
    }
    
    public LocalDate fechaLlegadaA_SiSaleEl(TerminalPortuaria destino, LocalDate fechaInicio) {
    ...
    public boolean estanEnElRecorridoEntre(TerminalPortuaria origenBuscado, TerminalPortuaria destinoBuscado, TerminalPortuaria origen, TerminalPortuaria destino) {        
        int indexOrigenBuscado = puertos.indexOf(origenBuscado);
        int indexDestinoBuscado = puertos.indexOf(destinoBuscado);
        int indexOrigen = puertos.indexOf(origen);
        int indexDestino = puertos.indexOf(destino);
        
        return indexOrigenBuscado >= indexOrigen && indexDestinoBuscado <= indexDestino;
    }
